...
describe('Pagination', () => {
  ...
  it('not renders next when last page', () => {
    render(<Pagination currentPage={10} numPages={10} middleNumPages={4} />);
    expect(
      screen.queryByRole('link', { name: '前のページへ' })
    ).toBeInTheDocument();
    expect(
  ...
  it('renders all pages if numPages is smaller than middlePages', () => {
    ...
    expect(screen.queryByRole('link', { name: 'ページ1' })).toBeInTheDocument();
    expect(screen.queryByRole('link', { name: 'ページ2' })).toBeInTheDocument();
    expect(screen.queryByRole('link', { name: 'ページ3' })).toBeInTheDocument();
    expect(screen.queryByRole('link', { name: 'ページ4' })).toBeInTheDocument();
    expect(screen.queryByRole('link', { name: 'ページ5' })).toBeInTheDocument();
    expect(screen.queryByRole('link', { name: 'ページ6' })).toBeInTheDocument();
    expect(screen.queryByRole('link', { name: 'ページ7' })).toBeInTheDocument();
    expect(
